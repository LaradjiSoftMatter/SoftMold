...
int main(int argc, char **argv)
{
	...
	while(xyzFile.load())
	{
		...
		if(time>=fromTime)
		{
			...
			//double param=1.0/(size.x*size.y);//pow(size.x*size.y,2.0)/(n.x*n.y);
			//double param=(r.x*r.y)/(n.x*n.y);
			double param=(r.x*r.y)/(size.x*size.y);
			std::vector<double> sF(n.x,0.0);
			//std::vector<double> fc;
			//for(int i=0;i<n.x;i++)
			//	fc.push_back(static_cast<double>(i)*M_PI*2.0/size.x);
			...
			for(int i=1;i<n.x/2;i++)
				if(count[i]!=0)
					structureFactor[i]+=sF[i]/static_cast<double>(count[i]);
	...
	std::vector<double> fc;
	for(int i=0;i<n.x;i++)
		fc.push_back(static_cast<double>(i)*2.0*M_PI/(avgSize));
	
	for(int i=1;i<n.x/2;i++)
		if(frames!=0)
			std::cout << fc[i] << '\t' << (structureFactor[i]/(static_cast<double>(frames))) << std::endl;
		else
			std::cout << fc[i] << '\t' << 0 << std::endl;
