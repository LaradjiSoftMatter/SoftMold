...
int main(int argc, char **argv)
{
	if(argc!=4)
	{
		std::cout << argv[0] << " name storeInterval measureInterval\n";
		return 0;
	}
	
	double storeInterval,measureInterval;
	std::stringstream cmdArgs;
	for(int i=2;i<4;i++)
		cmdArgs << argv[i] << ' ' ;
	cmdArgs >> storeInterval >> measureInterval;
	...
	//Set a new time
	System.setMeasureInterval(measureInterval);
	System.setStoreInterval(storeInterval);
