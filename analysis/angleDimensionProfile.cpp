...
int main(int argc, char* argv[])
{
	if(argc!=6)
	{
		std::cerr << "\tUsage: " << argv[0] << " name fromTime thickness dimension molIndex\n";
	...
	int mIndex;
	double thickness,fromTime;
	std::string dimStr;
	cmdArg >> fromTime >> thickness >> dimStr >> mIndex;
	int dim=-1;
	...
	std::vector<std::vector<double>> meanAngles;
	...
	while(xyzFile.load())
	{
		...
		for(auto bP:bondPairs)
		{
			...
			double theta=dotProduct(d,zHat);
		...
		{
			double mean=0;
			for(auto theta:slabs[i])
				mean+=theta;
			mean/=slabs[i].size();
			if(slabs[i].size()>0)
				std::cout << i*thickness << '\t' << mean << '\n';
			while(meanAngles.size()<=i)
				meanAngles.push_back(std::vector<double>());
			
			if(slabs[i].size()>0)
				meanAngles[i].push_back(mean);
			else
				meanAngles[i].push_back(0.0);
		}
		std::cout << std::endl;
		}
	...
	for(auto mA:meanAngles)
	{
		...
		mean/=mA.size();
		if(mA.size()>0)
			std::cout << slabIndex*thickness << '\t' << mean << '\n';
		slabIndex++;
	}
	//close our old files
	xyzFile.close();
