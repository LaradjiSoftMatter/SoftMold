...
int main(int argc, char **argv)
{
	...
	while(xyzFile.load())
	{
		...
		if(time>=fromTime)
		{
			std::vector<threeVector<double>> rogs;
			std::vector<double> rogXYs;
			for(auto chain:chains)
				rogs.push_back(radiusOfGyrationAxisIndex(chain.begin(),chain.end(),p,size));
			for(auto chain:chains)
				rogXYs.push_back(radiusOfGyrationBasisIndex(chain.begin(),chain.end(),p,size,xyBasis));
			
			threeVector<double> rogAvg=0;
			for(auto rog:rogs)
			{
				rogAvg.x+=rog.x;
				rogAvg.y+=rog.y;
			...
			for(auto rog:rogXYs)
				rogXYavg+=rog;
			if(rogXYs.size()>0)
				rogXYavg/=rogXYs.size();
			
			frames++;
			std::cerr << time << ' ' << rogAvg.x << ' ' << rogAvg.y << ' ' << rogAvg.z << ' ' << rogXYavg << std::endl;
			std::cout << time << ' ' << rogAvg.x << ' ' << rogAvg.y << ' ' << rogAvg.z << ' ' << rogXYavg << std::endl;
		}
