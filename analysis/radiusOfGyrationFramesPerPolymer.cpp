...
int main(int argc, char **argv)
{
	if(argc!=4 && argc!=5)
	{
		...
		std::cerr << argv[0] << " name fromTime molIndex bondIndex\n";
		return 0;
	}
	...
	while(xyzFile.load())
	{
		...
		if(time>=fromTime)
		{
			...
			double rogAvg=0,rogMax=rogs[0],rogMin=rogs[0];
			for(auto rog:rogs)
			{
				rogAvg+=rog;
				if(rog>rogMax) rogMax=rog;
				if(rog<rogMin) rogMin=rog;
			}
			if(rogs.size()>0)
				rogAvg/=rogs.size();
			...
			if(bondIndex>=0)
				std::cout << time << ' ' << rogs[bondIndex] << std::endl;
			else
			{
				//std::cout << time << ' ';
				for(auto rog:rogs)
