...
int main(int argc, char **argv)
{
	...
	int frames=0;
	std::vector<double> avgROGs,stdROGs;
	...
	double rogAvg=0,rogStd=0;
	for(auto rog:avgROGs)
		rogAvg+=rog;
	if(avgROGs.size()>0)
		rogAvg/=avgROGs.size();
	
	for(auto rog:avgROGs)
		rogStd+=(rog-rogAvg)*(rog-rogAvg);
	if(avgROGs.size()>0)
		rogStd/=avgROGs.size();
	rogStd=std::sqrt(rogStd);
	std::cout << rogAvg << ' ' << rogStd << std::endl;
