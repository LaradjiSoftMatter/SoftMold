...
void showSwitches(int argc, char **argv)
{
	...
	std::cerr << "\tAssume a circular perimeter for integral weighting." << std::endl;
	
	std::cerr << "--spherical" << std::endl;
	std::cerr << "\tAssume a spherical area for integral weigting." << std::endl;
...
int main(int argc, char **argv)
{
	...
	for(int i=1;i<argc;i++)
		cmdArg << argv[i] << ' ';
	enum SWITCHESS {DELTAX_S, NOX_S, FIRSTX_S, LASTX_S, SUMONLY_S, CIRCULAR_S, SPHERICAL_S, CONSTANT_S, HELP_S};
	...
	switches[std::string("--circular")]=CIRCULAR_S;
	switches[std::string("--spherical")]=SPHERICAL_S;
	...
	bool noX=false, fX=false, lX=false, circular=false, spherical=false;
	...
	for(int i=0;i<v.size();i++)
	{
		...
		vAvg.y()+=v[i].y();
		vSum.x()=v[i].x();
		double yVal=(v[i].y()+v[i-1].y())*0.5;
		double xVal=v[i].x()-v[i-1].x();
		if(circular)
			yVal*=M_PI*v[i].x();
		if(spherical)
			yVal*=v[i].x()*v[i].x();
	...
	if(v.size()>0)
		vAvg.x()/=vAvg.y();//*static_cast<double>(v.size());
	std::cerr << "R: " << vAvg.x() << std::endl;
	if(sumOnlyPrefix.length()>0)
	{
		if(spherical)
			std::cout << sumOnlyPrefix << ' ' << (vSum.y()+constant)/(vAvg.x()*vAvg.x()) << std::endl;
		else
