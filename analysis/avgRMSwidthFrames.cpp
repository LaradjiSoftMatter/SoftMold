		...
		if(time>=fromTime)
		{
			...
			for(auto &cell:cellList)
			{
				for(auto &particle:cell)
				{
					if(cell.size()!=0)
					{
						avg1+=particle.s[dimension]/cell.size();
						avg2+=particle.s[dimension]*particle.s[dimension]/(cell.size()*cell.size());
					}
				}
			}
			double totalAvg=avg1/(size.s[(dimension+1)%3]*size.s[(dimension+2)%3]);
