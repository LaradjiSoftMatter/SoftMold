...
int main(int argc, char* argv[])
{
	if(argc!=9)
	{
		...
		std::cout << "Usage: " << argv[0] << " oldName newName vertices.dat edges.dat kBond pos.x pos.y pos.z\n";
		return 0;
	}
	...
	//System.allocMolecule(edges.size());
	...
	for(auto &edge:edges)
	{
		...
		d.x=vertices[edge.s[0]].x-vertices[edge.s[1]].x;
		d.y=vertices[edge.s[0]].y-vertices[edge.s[1]].y;
		d.z=vertices[edge.s[0]].z-vertices[edge.s[1]].z;
		double lBond=std::sqrt(d.x*d.x+d.y*d.y+d.z*d.z);
		mBonds.addConstant(lBond);
		mBonds.addConstant(kBond);
		edge.s[0]+=newParticleOffset;
		edge.s[1]+=newParticleOffset;
		mBonds.addBond(edge);
