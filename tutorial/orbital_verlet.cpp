...
int main(int argc, char **argv)
{
	...
	output0.open("p_verlet0.dat", ios::out);
	output1.open("p_verlet1.dat", ios::out);

	if(!output0.is_open() || !output1.is_open())
	{
		cout << "Cannot open p_verlet0.dat or p_verlet1.dat!\n";
		output0.close();
		output1.close();
		return 0;
	}
	...
	for(int i=0;i<nSteps;i++)
	{
		...
		p[0].x+=v[0].x*dT+a[0].x*dT*dT/2.0;
		p[0].y+=v[0].y*dT+a[0].y*dT*dT/2.0;
		p[1].x+=v[1].x*dT+a[1].x*dT*dT/2.0;
		p[1].y+=v[1].y*dT+a[1].y*dT*dT/2.0;
		...
		v[0].x=vHalf[0].x+(aHalf[0].x+a[0].x)*dT/2.0;
		v[0].y=vHalf[0].y+(aHalf[0].y+a[0].y)*dT/2.0;
		v[1].x=vHalf[1].x+(aHalf[1].x+a[1].x)*dT/2.0;
		v[1].y=vHalf[1].y+(aHalf[1].y+a[1].y)*dT/2.0;
