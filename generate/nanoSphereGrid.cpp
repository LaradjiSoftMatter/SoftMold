...
int main(int argc, char **argv)
{
	...
	{
		int xIndex=i%xSide;
		int yIndex=floor(i/xSide);
		int xOffset=(xIndex%2)*nanoRadius;
		unitCell[0].x=0;
		unitCell[0].y=0;
		unitCell[0].z=0;
		...
		toPlace.x=static_cast<double>(xIndex)*2.0*nanoRadius+System.readSize().x/2.0-static_cast<double>(xSide)*nanoRadius;
		toPlace.y=static_cast<double>(yIndex)*2.0*nanoRadius+xOffset+System.readSize().y/2.0-static_cast<double>(xSide)*nanoRadius;
		toPlace.z=pos.z+nanoRadius+1.0+3.0*2.0*0.7;
		
		nanoSphere<double>(System, toPlace, nanoRadius, 0.5*1.31345933134, unitCell, 4, bonded, 12, constants, 2);
	}
