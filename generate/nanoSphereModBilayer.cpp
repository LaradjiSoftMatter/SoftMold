...
int main(int argc, char **argv)
{
	if(argc!=13)
	{
		...
		std::cout << "nanoRadius nanoHeadUmin nNanoparticles nanoNanoUmin nanoHead2Umin tailTail2Umin tailMaskR\n";
		return 0;
	}
	...
	double nanoHead2Umin=atof(argv[10]);
	double tailTail2Umin=atof(argv[11]);
	double tailMaskR=atof(argv[12]);
	...
	for(int i=nanoType;i<nanoType+nNanoparticles;i++)
	{
		for(int j=nanoType;j<nanoType+nNanoparticles;j++)
		{
			if(i!=j)
			{
	...
	//Add a nanoparticle
	//could use different nanotypes at different lattice positions
